...
for /f "delims=" %%a in ('call ExtractData.bat ../project.cfg ../user.cfg CoreModName') do (
	set modNameCore=%%a
)

for /f "delims=" %%a in ('call ExtractData.bat ../project.cfg ../user.cfg VehiclesModName') do (
	set modNameVehicles=%%a
)
...
echo CoreModName is: "%modNameCore%"
if "%modNameCore%"=="" (
	set /a failed=1
	echo CoreModName parameter was not set in the project.cfg
)

echo VehiclesModName is: "%modNameVehicles%"
if "%modNameVehicles%"=="" (
	set /a failed=1
	echo VehiclesModName parameter was not set in the project.cfg
)
...
echo removing DZEXPANSIONCORE....
if exist "P:\DZEXPANSIONCORE" (rmdir /s/q "P:\DZEXPANSIONCORE")
if ERRORLEVEL 1 goto:eof

echo removing DZEXPANSIONVEHICLES....
if exist "P:\DZEXPANSIONVEHICLES" (rmdir /s/q "P:\DZEXPANSIONVEHICLES")
if ERRORLEVEL 1 goto:eof
...
extractpbodos -p "%modBuildDirectory%%modNameCore%\Addons" P:\DZEXPANSIONCORE
extractpbodos -p "%modBuildDirectory%%modNameVehicles%\Addons" P:\DZEXPANSIONVEHICLES
...
cd /d %workDrive%/DZEXPANSIONCORE
for /R %%D in ( config.cpp ) do (
     IF EXIST "%%D" (
        set _CONFIG=%%~fD
        set _PREFIX=%%~pD

        set _PREFIX=!_PREFIX:\DZEXPANSIONCORE=!
...
cd /d %workDrive%/DZEXPANSIONVEHICLES
for /R %%D in ( config.cpp ) do (
     IF EXIST "%%D" (
        set _CONFIG=%%~fD
        set _PREFIX=%%~pD

        set _PREFIX=!_PREFIX:\DZEXPANSIONVEHICLES=!
...
xcopy /Y /S /Q "P:\DZEXPANSIONVEHICLES\DayZExpansion\Vehicles\Scripts" "%gitDiffDirectory%\DayZExpansion\Vehicles\Scripts\"
xcopy /Y /S /Q "P:\DZEXPANSIONCORE\DayZExpansion\Core" "%gitDiffDirectory%\DayZExpansion\Core\"
xcopy /Y /S /Q "P:\DZEXPANSIONCORE\DayZExpansion\GUI" "%gitDiffDirectory%\DayZExpansion\GUI\"
